...
```csharp
IComplianceController complianceController = client.ComplianceController;
```
...
[`Task<Models.IdvCheckListResult>`](../../doc/models/idv-check-list-result.md)
...
try
{
    IdvCheckListResult result = await complianceController.ListIdentityChecksAsync(userToken);
}
...
[`Task<Models.IdvCheckResult>`](../../doc/models/idv-check-result.md)
...
try
{
    IdvCheckResult result = await complianceController.RetrieveIdentityCheckAsync(
        userToken,
        idvcToken
    );
}
